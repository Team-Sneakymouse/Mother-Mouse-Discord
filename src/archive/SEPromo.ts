...
export default function SEPromo(client: Client, pocketbase: PocketBase) {
	...
	socket.on("partner-integrations:event", async (data: any) => {
		...
		const eventMapping = await pocketbase
			.collection<EventMappingRecord>("msd_event_mapping")
			.getFirstListItem(`event="${eventCode}"`)
			.catch((e) => {
				if (e.status === 404) {
					return {
						text: eventCode,
		...
		if (debugChannel) debugChannel.send(`${username} achieved: ${eventMapping.text}:\`\`\`json\n${JSON.stringify(data, null, 2)}\`\`\``);

		if (infoChannel && eventMapping.broadcast) await infoChannel.send(`**${username}** has achieved **${eventMapping.text}**`);
	});
}
